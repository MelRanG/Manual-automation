...
export async function createSession(config: WidgetConfig, anonymousId: string) {
  const res = await fetch(`${config.apiUrl}/api/widget/sessions`, {
    ...
    body: JSON.stringify({ site_id: config.siteId, anonymous_id: anonymousId }),
  })
...
export async function* askStream(config: WidgetConfig, sessionId: string, question: string) {
  ...
  for await (const event of parseSSE(res)) {
    if (event.event === "token") {
      ...
      yield { type: "citations" as const, citations: data.citations, warnings: data.warnings }
    } else if (event.event === "done") {
      const { message_id } = JSON.parse(event.data)
      yield { type: "done" as const, messageId: message_id as string }
    }
  }
}
